  ...
  setup() {
    ...
    const fetchPerson = async (id) => {
      try {
        ...
        errorMessage.value = 'Erreur lors de la récupération de la personne.';
      }
    };

    // Fonction pour mettre à jour les détails de la personne
    const updatePerson = async () => {
      const age = calculateAge(person.value.dateNaissance); // Calcul de l'âge à partir de la date de naissance
      if (age > 150) {
        ageError.value = 'Les personnes de plus de 150 ans ne peuvent pas être enregistrées.'; // Vérification de l'âge maximum
        return;
      }
      ...
      try {
        ...
        errorMessage.value = 'Erreur lors de la modification de la personne.';
      }
    };
    ...
    return { person, ageError, errorMessage, updatePerson, validateAge, calculateAge };
  },
};
</script>
